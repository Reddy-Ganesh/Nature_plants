...
function login() {
    ...
    if (loggedIn) {
        ...
        if (screen1 <= 412) {
            ...
            memu1.style.left="110px"
           
           // toggle i predefine

            console.log("hello")
        }
        else if((screen1 > 412) &&(screen1 <= 612)) {
            icon_div.style.width = "270px"
            memu1.style.left = "55%"
            console.log("ganesh125")
...
function logout() {
    var screen1 = screen.width
    if ((screen1 <= 412) || (screen1 > 412) && (screen1 <= 612) ){
    ...
    // Loop through child elements of 'cart' and remove them except the close button
    for (let i = cart.children.length - 1; i >= 0; i--) {
        var childElement = cart.children[i];

        // Check if the child element is not the close button
        if (childElement !== cross) {
            cart.removeChild(childElement); // Remove the child element
        }
    }
